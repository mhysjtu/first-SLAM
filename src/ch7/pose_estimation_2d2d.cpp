...
int main(int argc, char **argv){
    ...
    cout<<"F: K-T*t^*R*K-1="<<endl<<((K.inv()).t())*t_x*R*(K.inv())<<endl;//it seems not working
    ...
    for	( DMatch m: matches ){
    	...
    	Mat d = y2.t() * t_x * R * y1;
    	cout<<"epipolar constraint (through E)= "<<d<<endl;

    	Mat y1F = (Mat_<double> (3,1) << keypoints_1[m.queryIdx].pt.x, keypoints_1[m.queryIdx].pt.y, 1);
    	Mat y2F = (Mat_<double> (3,1) << keypoints_2[m.trainIdx].pt.x, keypoints_2[m.trainIdx].pt.y, 1);
    	Mat dF = y2F.t() * ((K.inv()).t()) * t_x * R * (K.inv()) * y1F;
    	cout<<"epipolar constraint (through F)= "<<dF<<endl;//the result is similar to the previous one
    }
